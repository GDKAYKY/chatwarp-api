...
impl PostgresStore {
    ...
    pub async fn create_new_device(&self) -> Result<i32> {
        ...
        tokio::task::spawn_blocking(move || -> Result<i32> {
            ...
            let new_device = warp_core::store::Device::new();
    ...
    pub async fn load_device_data_for_device(&self, device_id: i32) -> Result<Option<CoreDevice>> {
        ...
        )) = row
        {
            ...
            Ok(Some(CoreDevice {
                ...
                app_version_last_fetched_ms,
                device_props: {
                    use warp_core::store::device::DEVICE_PROPS;
                    DEVICE_PROPS.clone()
                },
                edge_routing_info,
            }))
...
impl DeviceStore for PostgresStore {
    async fn save(&self, device: &CoreDevice) -> Result<()> {
        PostgresStore::save_device_data_for_device(self, self.device_id, device).await
    }

    async fn load(&self) -> Result<Option<CoreDevice>> {
        PostgresStore::load_device_data_for_device(self, self.device_id).await
    }

    async fn exists(&self) -> Result<bool> {
        PostgresStore::device_exists(self, self.device_id).await
    }
