...
use warp_core::xml::DisplayableNode;
...
pub struct Client {
    pub(crate) core: warp_core::client::CoreClient,
    ...
    pub(crate) device_registry_cache: Cache<String, warp_core::store::traits::DeviceListRecord>,
...
impl Client {
    pub async fn new(
        ...
        let core = warp_core::client::CoreClient::new(device_snapshot.core.clone());
    ...
    pub async fn fetch_props(&self) -> Result<(), crate::request::IqError> {
        ...
        let iq = InfoQuery::get(
            "w",
            server_jid(),
            Some(warp_core_binary::node::NodeContent::Nodes(vec![props_node])),
        );
    ...
    pub(crate) async fn handle_success(self: &Arc<Self>, node: &warp_core_binary::node::Node) {
    ...
    pub(crate) async fn process_app_state_sync_task(
        ...
        if has_more {
            ...
            let iq = crate::request::InfoQuery {
                ...
                target: None,
                id: None,
                content: Some(warp_core_binary::node::NodeContent::Nodes(vec![sync_node])),
                timeout: None,
            };
            ...
            let pre_downloaded_snapshot: Option<Vec<u8>> =
                match warp_core::appstate::patch_decode::parse_patch_list(&resp) {
                    Ok(pl) => {
    ...
    pub(crate) async fn handle_stream_error(&self, node: &warp_core_binary::node::Node) {
        self.is_logged_in.store(false, Ordering::Relaxed);
    ...
    pub(crate) async fn handle_connect_failure(&self, node: &warp_core_binary::node::Node) {
        self.expected_disconnect.store(true, Ordering::Relaxed);
        self.shutdown_notifier.notify_waiters();
        ...
        if reason.is_logged_out() {
            ...
            self.core
                .event_bus
                .dispatch(&warp_core::types::events::Event::LoggedOut(
    ...
    pub(crate) async fn handle_iq(self: &Arc<Self>, node: &warp_core_binary::node::Node) -> bool {
...
mod tests {
    ...
    use warp_core_binary::jid::SERVER_JID;
    ...
    ///
    /// This is the method used by warp_core::send to look up LID mappings when encrypting.
    #[tokio::test]
