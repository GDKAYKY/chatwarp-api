...
use warp_core::appstate::keys::ExpandedAppStateKeys;
use warp_core::appstate::patch_decode::{PatchList, WAPatchName, parse_patch_list};
use warp_core::appstate::{
    collect_key_ids_from_patch_list, expand_app_state_keys, process_patch, process_snapshot,
};
use warp_core::store::traits::Backend;
use warp_core_binary::node::Node;
use waproto::whatsapp as wa;

// Re-export Mutation from warp_core for backwards compatibility
pub use warp_core::appstate::Mutation;
...
impl AppStateProcessor {
    ...
    pub async fn process_patch_list(
        ...
        if let Some(snapshot) = &pl.snapshot {
            ...
            let get_keys =
                |key_id: &[u8]| -> Result<ExpandedAppStateKeys, warp_core::appstate::AppStateError> {
                    use base64::Engine;
                    use base64::engine::general_purpose::STANDARD_NO_PAD;
                    let id_b64 = STANDARD_NO_PAD.encode(key_id);
                    key_cache
                        .get(&id_b64)
                        .cloned()
                        .ok_or(warp_core::appstate::AppStateError::KeyNotFound)
                };
        ...
        for patch in &pl.patches {
            ...
            let get_prev_value_mac = |index_mac: &[u8]| -> Result<
                Option<Vec<u8>>,
                warp_core::appstate::AppStateError,
...
mod tests {
    ...
    use prost::Message;
    use std::collections::HashMap;
    use warp_core::appstate::WAPATCH_INTEGRITY;
    use warp_core::appstate::hash::HashState;
    use warp_core::appstate::hash::generate_content_mac;
    use warp_core::appstate::keys::expand_app_state_keys;
    use warp_core::appstate::processor::AppStateMutationMAC;
    use warp_core::libsignal::crypto::aes_256_cbc_encrypt_into;
    use warp_core::store::error::Result as StoreResult;
    ...
    impl DeviceStore for MockBackend {
        async fn save(&self, _: &warp_core::store::Device) -> StoreResult<()> {
            Ok(())
        }
        async fn load(&self) -> StoreResult<Option<warp_core::store::Device>> {
            Ok(Some(warp_core::store::Device::new()))
        }
    ...
    fn create_encrypted_mutation(
        ...
        index_mac: &[u8],
        plaintext: &[u8],
        keys: &warp_core::appstate::keys::ExpandedAppStateKeys,
        key_id_bytes: &[u8],
    ) -> wa::SyncdMutation {
        let iv = vec![0u8; 16];
