...
impl From<warp_core::request::IqError> for IqError {
    fn from(err: warp_core::request::IqError) -> Self {
        match err {
            warp_core::request::IqError::Timeout => Self::Timeout,
            warp_core::request::IqError::NotConnected => Self::NotConnected,
            warp_core::request::IqError::Disconnected(node) => Self::Disconnected(node),
            warp_core::request::IqError::ServerError { code, text } => {
                Self::ServerError { code, text }
            }
            warp_core::request::IqError::InternalChannelClosed => Self::InternalChannelClosed,
            warp_core::request::IqError::Network(msg) => Self::Socket(SocketError::Crypto(msg)),
        }
    }
}
...
impl Client {
    ...
    ///
    /// ```rust,no_run
    /// use warp_core::request::{InfoQuery, InfoQueryType};
