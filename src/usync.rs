    ...
    pub(crate) async fn get_user_devices(&self, jids: &[Jid]) -> Result<Vec<Jid>, anyhow::Error> {
        ...
        if !jids_to_fetch.is_empty() {
            ...
            for user_list in &user_device_lists {
                ...
                let device_list = warp_core::store::traits::DeviceListRecord {
                    user: user_list.user.user.clone(),
                    devices: user_list
                        .devices
                        .iter()
                        .map(|d| warp_core::store::traits::DeviceInfo {
...
#[cfg(test)]
mod tests {
    include!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/tests/usync_tests.rs"));
}
