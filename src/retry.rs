...
use warp_core::libsignal::store::PreKeyStore;
...
impl Client {
    ...
    async fn process_retry_key_bundle(
        &self,
        node: &Node,
        requester_jid: &warp_core_binary::jid::Jid,
        is_peer: bool,
    ) -> Result<(), anyhow::Error> {
        let keys_node = node
...
mod tests {
    ...
    async fn base_key_store_operations() {
        use warp_core::store::traits::ProtocolStore as _;
