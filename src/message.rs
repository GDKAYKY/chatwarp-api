...
use warp_core::libsignal::protocol::process_sender_key_distribution_message;
...
use warp_core::libsignal::store::sender_key_name::SenderKeyName;
use warp_core::messages::MessageUtils;
...
impl Client {
    ...
    pub(crate) async fn handle_encrypted_message(self: Arc<Self>, node: Arc<Node>) {
        ...
        let sender_encryption_jid = {
            ...
            let pn_server = warp_core_binary::jid::DEFAULT_USER_SERVER;
            let lid_server = warp_core_binary::jid::HIDDEN_USER_SERVER;
    ...
    async fn process_session_enc_batch(
        self: Arc<Self>,
        enc_nodes: &[&warp_core_binary::node::Node],
        info: &MessageInfo,
        sender_encryption_jid: &Jid,
    ) -> (bool, bool, bool) {
        // Returns (any_success, any_duplicate, dispatched_undecryptable)
        use warp_core::libsignal::protocol::CiphertextMessage;
        ...
        for enc_node in enc_nodes {
            let ciphertext: &[u8] = match &enc_node.content {
                Some(warp_core_binary::node::NodeContent::Bytes(b)) => b,
                _ => {
    ...
    pub(crate) async fn parse_message_info(
        ...
        let mut source = if from.server == warp_core_binary::jid::BROADCAST_SERVER {
            ...
            crate::types::message::MessageSource {
                ...
                broadcast_list_owner: if from.user != warp_core_binary::jid::STATUS_BROADCAST_USER {
            ...
            let sender_alt = if from.server == warp_core_binary::jid::HIDDEN_USER_SERVER {
...
mod tests {
    ...
    async fn test_process_session_enc_batch_handles_session_not_found_gracefully() {
        use warp_core::libsignal::protocol::{IdentityKeyPair, KeyPair, SignalMessage};
    ...
    fn test_lid_protocol_address_consistency() {
        use warp_core::types::jid::JidExt as CoreJidExt;
        use warp_core_binary::jid::Jid;
    ...
    ///
    /// This is a unit test for the logic in warp_core/src/send.rs that converts
    ...
    fn test_lid_to_phone_mapping_for_device_queries() {
        ...
        use warp_core::types::message::AddressingMode;
        use warp_core_binary::jid::Jid;
    ...
    fn test_own_jid_check_in_lid_mode() {
        ...
        // Simulate the own JID check logic from warp_core/src/send.rs
    ...
    async fn test_sender_key_always_uses_display_jid() {
        use crate::store::SqliteStore;
        use std::sync::Arc;
        use warp_core::libsignal::protocol::{SenderKeyStore, create_sender_key_distribution_message};
        use warp_core::libsignal::store::sender_key_name::SenderKeyName;
    ...
    async fn test_second_message_with_only_skmsg_decrypts() {
        ...
        let skmsg_ciphertext = {
            let mut device_guard = device_arc.write().await;
            let sender_key_msg = warp_core::libsignal::protocol::group_encrypt(
                &mut *device_guard,
                &sender_key_name,
                b"ping",
                &mut rand::rngs::OsRng.unwrap_err(),
    ...
    async fn test_untrusted_identity_does_not_break_batch_processing() {
        ...
        let enc_node_refs: Vec<&warp_core_binary::node::Node> = enc_nodes.iter().collect();
    ...
    async fn test_pn_message_uses_lid_for_session_lookup_when_mapping_known() {
        ...
        let dm_node_with_sender_lid = warp_core_binary::builder::NodeBuilder::new("message")
            .attr("from", Jid::pn(phone).to_string())
            .attr("sender_lid", Jid::lid(lid).to_string())
    ...
    async fn test_pn_message_uses_cached_lid_without_sender_lid_attribute() {
        ...
        let dm_node_without_sender_lid = warp_core_binary::builder::NodeBuilder::new("message")
            .attr("from", Jid::pn(phone).to_string())
            // Note: No sender_lid attribute!
    ...
    async fn test_pn_message_uses_pn_when_no_lid_mapping() {
        ...
        let dm_node = warp_core_binary::builder::NodeBuilder::new("message")
            .attr("from", Jid::pn(phone).to_string())
    ...
    fn create_test_message_info(chat: &str, msg_id: &str, sender: &str) -> MessageInfo {
        use warp_core::types::message::{EditAttribute, MessageSource, MsgMetaInfo};
