...
impl Client {
    ...
    pub(crate) async fn update_device_list(
        &self,
        mut record: warp_core::store::traits::DeviceListRecord,
    ) -> Result<()> {
        use anyhow::Context;
...
mod tests {
    ...
    async fn test_has_device_with_cached_record() {
        ...
        let record = warp_core::store::traits::DeviceListRecord {
            user: lid.to_string(),
            devices: vec![warp_core::store::traits::DeviceInfo {
                device_id: 1,
                key_index: None,
            }],
            timestamp: 12345,
    ...
    async fn test_invalidate_device_cache_uses_correct_jid_types() {
        ...
        // Re-insert entries
        let record2 = warp_core::store::traits::DeviceListRecord {
            user: lid.to_string(),
            devices: vec![warp_core::store::traits::DeviceInfo {
                device_id: 2,
                key_index: None,
