...
impl Client {
    ...
    async fn handle_placeholder_resend_response(
        ...
        self.core
            .event_bus
            .dispatch(&warp_core::types::events::Event::Message(
                Box::new(message),
                message_info,
            ));
    }
    ...
    pub(crate) fn spawn_pdo_request_with_options(
        ...
        if info.source.chat.server == warp_core_binary::jid::BROADCAST_SERVER {
            return;
        }
...
#[cfg(test)]
mod tests {
    include!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/tests/pdo_tests.rs"));
}
