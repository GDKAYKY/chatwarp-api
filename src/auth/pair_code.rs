...
//!
//! # async fn example(client: std::sync::Arc<chatwarp_api::Client>) -> Result<(), Box<dyn std::error::Error>> {
...
use warp_core::pair_code::{PairCodeError, PairCodeState, PairCodeUtils};
use warp_core_binary::jid::{Jid, SERVER_JID};
use warp_core_binary::node::{Node, NodeContent};

// Re-export types for user convenience
pub use warp_core::pair_code::{PairCodeOptions, PlatformId};
...
impl Client {
    ...
    pub async fn pair_with_code(
        ...
        let ephemeral_keypair = KeyPair::generate(&mut rand::rngs::StdRng::from_os_rng());
...
pub(crate) async fn handle_pair_code_notification(client: &Arc<Client>, node: &Node) -> bool {
    ...
    // commented out, so pairing works without it. See warp_core/src/pair.rs:147-153.
