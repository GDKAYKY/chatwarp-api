...
pub use warp_core::prekeys::PreKeyUtils;
...
impl Client {
    ...
    pub(crate) async fn get_server_pre_key_count(&self) -> Result<usize, crate::request::IqError> {
        ...
        let iq = InfoQuery::get(
            "encrypt",
            server_jid(),
            Some(warp_core_binary::node::NodeContent::Nodes(vec![count_node])),
        );
    ...
    pub(crate) async fn upload_pre_keys(&self) -> Result<(), anyhow::Error> {
        ...
        let iq = InfoQuery::set(
            "encrypt",
            server_jid(),
            Some(warp_core_binary::node::NodeContent::Nodes(iq_content)),
        );
