...
pub use warp_core::download::{DownloadUtils, Downloadable, MediaType};

impl From<&MediaConn> for warp_core::download::MediaConnection {
    fn from(conn: &MediaConn) -> Self {
        warp_core::download::MediaConnection {
            hosts: conn
                .hosts
                .iter()
                .map(|h| warp_core::download::MediaHost {
...
impl Client {
    pub async fn download(&self, downloadable: &dyn Downloadable) -> Result<Vec<u8>> {
        ...
        let core_media_conn = warp_core::download::MediaConnection::from(&media_conn);
    ...
    async fn download_and_decrypt_with_request(
        &self,
        request: &warp_core::download::DownloadRequest,
    ) -> Result<Vec<u8>> {
...
mod tests {
    ...
    fn process_downloaded_media_ok() {
        ...
        let enc = warp_core::upload::encrypt_media(data, MediaType::Image)
    ...
    fn process_downloaded_media_bad_mac() {
        let data = b"Tamper";
        let mut enc = warp_core::upload::encrypt_media(data, MediaType::Image)
            .expect("encryption should succeed");
