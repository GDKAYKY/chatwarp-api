...
use warp_core::libsignal::store::record_helpers as warp_core_record;
use warp_core::libsignal::store::sender_key_name::SenderKeyName;
use warp_core::libsignal::store::{
    PreKeyStore as warp_corePreKeyStore, SignedPreKeyStore as warp_coreSignedPreKeyStore,
};
...
impl PreKeyStore for PreKeyAdapter {
    async fn get_pre_key(&self, prekey_id: PreKeyId) -> Result<PreKeyRecord, SignalProtocolError> {
        let device = self.0.device.read().await;
        warp_corePreKeyStore::load_prekey(&*device, prekey_id.into())
            .await
            .map_err(|e| SignalProtocolError::InvalidState("backend", e.to_string()))?
            .ok_or(SignalProtocolError::InvalidPreKeyId)
            .and_then(warp_core_record::prekey_structure_to_record)
    }
    async fn save_pre_key(
        ...
        let device = self.0.device.read().await;
        let structure = warp_core_record::prekey_record_to_structure(record)?;
        warp_corePreKeyStore::store_prekey(&*device, prekey_id.into(), structure, false)
            .await
            .map_err(|e| SignalProtocolError::InvalidState("backend", e.to_string()))
    }
    async fn remove_pre_key(&mut self, prekey_id: PreKeyId) -> Result<(), SignalProtocolError> {
        let device = self.0.device.read().await;
        warp_corePreKeyStore::remove_prekey(&*device, prekey_id.into())
            .await
            .map_err(|e| SignalProtocolError::InvalidState("backend", e.to_string()))
    }
}
...
impl SignedPreKeyStore for SignedPreKeyAdapter {
    async fn get_signed_pre_key(
        ...
        let device = self.0.device.read().await;
        warp_coreSignedPreKeyStore::load_signed_prekey(&*device, signed_prekey_id.into())
            .await
            .map_err(|e| SignalProtocolError::InvalidState("backend", e.to_string()))?
            .ok_or(SignalProtocolError::InvalidSignedPreKeyId)
            .and_then(warp_core_record::signed_prekey_structure_to_record)
    }
...
#[async_trait]
impl warp_core::libsignal::protocol::SenderKeyStore for SenderKeyAdapter {
    async fn store_sender_key(
        &mut self,
        sender_key_name: &SenderKeyName,
        record: &warp_core::libsignal::protocol::SenderKeyRecord,
    ) -> warp_core::libsignal::protocol::error::Result<()> {
        let mut device = self.0.device.write().await;
        warp_core::libsignal::protocol::SenderKeyStore::store_sender_key(
            &mut *device,
            sender_key_name,
            record,
        )
    ...
    async fn load_sender_key(
        ...
        Option<warp_core::libsignal::protocol::SenderKeyRecord>,
    > {
        let mut device = self.0.device.write().await;
        warp_core::libsignal::protocol::SenderKeyStore::load_sender_key(&mut *device, sender_key_name)
            .await
    }
}
