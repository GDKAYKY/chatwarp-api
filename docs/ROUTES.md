...
- `GET /instance/connect/:name` apenas dispara conexão e retorna `{"status":"connecting"}`.
- `GET /instance/:name/state` expõe estado atual, QR corrente e último erro.
